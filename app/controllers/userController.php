...
require_once __DIR__ . '/../../Libreria/PhpSpreadsheet/vendor/autoload.php';
...
class userController extends mainModel
{
    ...
    public function registrarUsuarioControlador()
    {
        ...
        $tipoUsuario = $this->limpiarCadena($_POST['usuario_tipoUsuario']);



        # Verificando campos obligatorios #
        if ($nombre == "" || $apellido == "" || $usuario == "" || $clave1 == "" || $clave2 == "" || $telefono == "" || $tipoUsuario == "") {
            $alerta = [
                "tipo" => "simple",
        ...
        # Validando teléfono #
        if ($this->verificarDatos("^[9][0-9]{8}$", $telefono)) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "El TELEFONO no coincide con el formato solicitado (debe ser 9 dígitos y comenzar con 9)",
                "icono" => "error"
            ];
            return json_encode($alerta);
            exit();
        ...
        $usuario_datos_reg = [
            ...
            [
                "campo_nombre" => "usuario_tipoUsuario",
    ...
    public function listarUsuarioControlador($pagina, $registros, $url, $busqueda)
    {
        ...
        $tabla .= '
		        <div class="table-container">
                    <form action="' . APP_URL . 'app/ajax/usuarioAjax.php" method="POST" style="display:inline;">
                        <input type="hidden" name="modulo_usuario" value="exportar">
                        <button type="submit" class="button is-primary is-rounded mb-3">
                            <span class="icon is-small">
                                <i class="fas fa-file-excel"></i> <!-- Ícono de Excel -->
                            </span>
                            <span>Reporte</span> <!-- Si deseas mantener el texto visible -->
                        </button>
                    </form>
            ...
            foreach ($datos as $rows) {
                $tabla .= '
						<tr class="has-text-centered" >
                            ...
                            <td>' . $rows['usuario_tipoUsuario'] . '</td>
    ...
    public function actualizarUsuarioControlador()
    {
        ...
        if ($this->verificarDatos("^[9]{1}[0-9]{8}$", $telefono)) { // Verificación para el teléfono
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "El TELÉFONO no coincide con el formato solicitado (debe iniciar con 9 y tener 9 dígitos)",
                "icono" => "error"
            ];
            return json_encode($alerta);
            exit();
    ...
    public function exportarUsuarios()
    {
        ...
        $headers = ['#', 'Nombre', 'Usuario', 'Email', 'Rol', 'Creado', 'Actualizado'];
        $columnWidths = [5, 30, 20, 40, 20, 35, 35];
        $rowHeight = 20;
        $headerHeight = 40;
        $fontBold = true;
        $fontSize = 12;
        ...
        foreach ($headers as $index => $header) {
            $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($index + 3);
            $sheet->setCellValue($columnLetter . $startRow, $header);
            $sheet->getColumnDimension($columnLetter)->setWidth($columnWidths[$index]);

            $sheet->getStyle($columnLetter . $startRow)->getFont()->setBold($fontBold)->setSize($fontSize);
            $sheet->getStyle($columnLetter . $startRow)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle($columnLetter . $startRow)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);

            $sheet->getStyle($columnLetter . $startRow)->getBorders()->getAllBorders()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
            $sheet->getStyle($columnLetter . $startRow)->getBorders()->getAllBorders()->getColor()->setARGB('000000');

            $sheet->getStyle($columnLetter . $startRow)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID);
            $sheet->getStyle($columnLetter . $startRow)->getFill()->getStartColor()->setARGB('D9D9D9');

            $sheet->getRowDimension($startRow)->setRowHeight($headerHeight);
        }

        $row = $startRow + 1;
        $contador = 1;
        foreach ($datos as $usuario) {
            $sheet->setCellValue('C' . $row, $contador);
            $sheet->setCellValue('D' . $row, $usuario['usuario_nombre'] . ' ' . $usuario['usuario_apellido']);
            $sheet->setCellValue('E' . $row, $usuario['usuario_usuario']);
            $sheet->setCellValue('F' . $row, $usuario['usuario_email']);
            $sheet->setCellValue('G' . $row, $usuario['usuario_tipoUsuario']);
            $sheet->setCellValue('H' . $row, date("d-m-Y h:i:s A", strtotime($usuario['usuario_creado'])));
            $sheet->setCellValue('I' . $row, date("d-m-Y h:i:s A", strtotime($usuario['usuario_actualizado'])));

            $sheet->getRowDimension($row)->setRowHeight($rowHeight);

            foreach (range('C', 'I') as $columnLetter) {
                $sheet->getStyle($columnLetter . $row)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle($columnLetter . $row)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
                ...
                $sheet->getStyle($columnLetter . $row)->getBorders()->getAllBorders()->getColor()->setARGB('000000');
            }
        ...
        $fileName = 'Lista de Usuarios ' . date('Y-m-d_His') . '.xlsx';
